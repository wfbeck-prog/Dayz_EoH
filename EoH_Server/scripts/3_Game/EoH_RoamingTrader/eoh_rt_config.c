class EoH_RT_GlobalConfig
{
	...
	static const string CONFIG_DIR = "$profile:EoH_Server/RoamingTrader";
	static const string CONFIG_PATH = "$profile:EoH_Server/RoamingTrader/Config.json";
	...
	bool Load()
	{
		if (!FileExist("$profile:EoH_Server"))
			MakeDirectory("$profile:EoH_Server");
